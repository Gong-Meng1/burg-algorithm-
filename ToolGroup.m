classdef ToolGroup < handle
    ...
    %   <a href="matlab:help matlab.ui.internal.desktop.ToolGroup.isWaiting">isWaiting</a>    
    ...
    properties (Dependent, Access = public, SetObservable, AbortSet)
        ...
        %   Note: programmatically specifying '' is a null op.  In other
        %   words, a selected tab cannot be unselected programmatically.
        SelectedTab
    end
    ...
    methods
        ...
        function this = ToolGroup(apptitle, appname)
            ...
            myToolstripSwingService = matlab.ui.internal.toolstrip.swing.ToolstripSwingService(this.PeerModelChannel);
            this.PeerModelManager = com.mathworks.peermodel.PeerModelManagers.getInstance(this.PeerModelChannel);
            % create MCOS toolstrip object with swing service
            this.MCOSToolstrip = matlab.ui.internal.toolstrip.Toolstrip(myToolstripSwingService);
            this.MCOSToolstrip.Tag = 'ToolGroupToolstrip';
            % refresh app when requested by swing service
            this.ListenersToolstripSwingService = addlistener(this.MCOSToolstrip.ToolstripSwingService,'RefreshToolGroup',@(es, ed) refreshToolGroup(this));
        ...
        function delete(this)
            ...
            %pause(1);this.MCOSToolstrip.ToolstripSwingService.Registry.getSize()
            this.close;
        end
        ...
        function set.SelectedTab(this, value)
            ...
            if isempty(value)
                % currently this is a null-op (no error)
                this.Peer.setCurrentTab('');
            elseif any(strcmp(value,this.TabNames))
                % after rendering, make sure swing tab exists (in case the mcos tab was added moment ago)
                if isToolGroupValid(this)
                    while size(getTabList(this.Peer.getWrappedComponent()))~=length(this.TabNames)
                        pause(0.01);
                    end
                end
                % use swing toolstrip directly without going through peer
                % nodes and force "cbSwingSelectedTabChanged" to update
                % SelectedTab property in TabGroup immediately.
        ...
        function addTabGroup(this, tabgroup)
            ...
            drawnow();
            % The drawnow introduces a race condition on close of the app
            % that can lead to an attempt to addTabs to an invalid object.
            if ~isvalid(this)
                return;
            end
        ...
        function open(this)
            ...
            % disable tab selection change event when a selected tab is removed
            setAutoSuccession(this.SwingToolstrip,false); 
            % must let swing service know swing toolgroup is present
            this.MCOSToolstrip.ToolstripSwingService.SwingToolGroup = this.Peer;
            ...
            if isempty(this.AttributeListener)
                addAttributeListener( this, {@cbSwingSelectedTabChanged, this} );
            end
        end
		...
		function showTearOffDialog(this, dialog, anchor, floating)
            ...
            if nargin==4
                showTearOffDialog(anchor,dialog,floating,this.MCOSToolstrip);
            else
                showTearOffDialog(anchor,dialog,this.MCOSToolstrip);
            end
        end
        
		function showFigureDialog(this, fig, anchor)
            % Method "showFigureDialog": 
            %
            %   Display a figure-based dialog below a toolstrip control.
            %
            %   Example:
            %       showFigureDialog(this, fig, anchor);
            %   where: 
            %       "fig" must be a "figure", not a "uifigure"
            %       "anchor" must be a "matlab.ui.internal.toolstrip.***"
            showFigureDialog(anchor, fig, this.MCOSToolstrip);
        end
        ...
        function hideViewTab(this)
            ...
            % Since we are hiding the "Tabs Position" drop down, we need to disable hiding the document bar,
            % since there is no way to get it back without the "View" tab.
        ...
        function setWaiting(this, state, comps)
            ...
            %
            %   Must be used after the app is open.
            md = com.mathworks.mlservices.MatlabDesktopServices.getDesktop;
        ...
        function state = isWaiting(this)
            % Method "isWaiting": 
            %
            %   Return the app "Waiting" state controlled by "setWaiting". 
            %
            %   Example:
            %       setWaiting(this, true);
            %       isWaiting(this) % return true
            state = this.IsWaiting;
        end        
        ...
        function successful = addFigure(this, fig)
            ...
            % This is an API to morph the figure ie. when run in matlab online
            % or live editor, the figure/gcf command creates an embedded
            % web figure and not a java figure. But web figures are not
            % supported in a toolGroup, hence we calls this API where in we
            % change the figure view from web to java, keeping the model
            % same (the handle to the figure does not change).
    ...
    methods (Hidden)
        ...
        function jh = getToolstripSwingComponent(this, comp)
            jh = this.MCOSToolstrip.ToolstripSwingService.Registry.getWidgetById(comp.getId());
        end
...
%         end
        ...
        function addTabs(this, tabgroup)
            ...
            for i = 1:length(tabgroup.Children)
                tab_id = tabgroup.Children(i).getId();
                jtab = this.MCOSToolstrip.ToolstripSwingService.Registry.getWidgetByPollingId(tab_id);
                if ~list.contains(jtab)
                    this.Peer.add(jtab);
                end
            end
        ...
        function updateClientMap(this, client)
            % Set selected tab when an existing client becomes active based
            % on the client map or update the clientmap when a new client
            % is added based on the SelectedTab property.
            k = findClientIndex(this, client);
            if k > 0
                if isempty(this.ClientMap{k,3})
                    this.ClientMap{k,3} = this.SelectedTab;
                else
                    this.Peer.setCurrentTab(this.ClientMap{k,3});
                end
            end
        end        
...
% ------------------------------------------------------------------------------
function cbSwingSelectedTabChanged(~,ed,toolgroup)
% Callback fired when the SELECTED_TAB attribute is changed in the swing
% toolstrip widget.  What the callbask does:
%       If both old and new tabs belong to the same tab group, set
%       SelectedTab to new tab (if different), otherwise, set old tab
%       group SelectedTab to [] and new tab group SelectedTab to new
%       tab (if different). If there is an active client, update client
%       map with the new tab even if it is a permanent tab.  "drawnow" in
%       the "SelectedTab" set function in TabGroup is temporarily disabled. 
% The swing event can be triggered in the following scenarios:
%   (1) User selects a new tab from UI clicking. 
%   (2) Developer programmatically changes SelectedTab of toolgroup (forced sync)
%   (3) Developer programmatically changes SelectedTab of tabgroup (forced sync)
%   (4) Developer calls "tabgroup.removeTab" on the selected tab
%       Here the swing event is generated by auto-selecting of the home tab.
% Note: (1) "ToolGroup.SelectedTab" reflects current swing tab selection
%       (2) Add/remove client with contextual tabgroup does not trigger event
    %% get old and new tabs from swing event
    oldtabname = char(ed.getOldValue);
    newtabname = char(ed.getNewValue);
    %disp(['old tab: ' oldtabname ', new tab: ' newtabname])
    %% if newtabname is empty, exit, otherwise, get new tab and its tab group
    % "newtabname" should never be empty in all scenarios.
    % "newtab" might not exist if a selected tab is removed immediately.
    ...
    %% deal with old selected tab
    % if the old selected tab is still in toolstrip, set selected tab to
    % empty in the tab group hosting the old selected tab. otherwise (empty
    % or not found or same as the new tab group), skip updating it.
