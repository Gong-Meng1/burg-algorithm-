...
if nargin < 2
   ...
   uilist = { ...
         ...
         { 'Style', 'text', 'string', 'Time range [min max] (s)', 'fontangle', Tools.fastif(length(EEG)>1, 'italic', 'normal') }, ...
         { 'Style', 'edit', 'string', '', 'enable', Tools.fastif(length(EEG)>1, 'off', 'on') }, ...
         { }, { 'Style', 'checkbox', 'string', '    ', 'enable', Tools.fastif(length(EEG)>1, 'off', 'on') },{ }, ...
         ...
         { 'Style', 'text', 'string', 'Point range (ex: [1 10])', 'fontangle', Tools.fastif(length(EEG)>1, 'italic', 'normal') }, ...
         { 'Style', 'edit', 'string', '', 'enable', Tools.fastif(length(EEG)>1, 'off', 'on') }, ...
         { }, { 'Style', 'checkbox', 'string', '    ', 'enable', Tools.fastif(length(EEG)>1, 'off', 'on') },{ }, ...
         ...
         { 'Style', 'text', 'string', 'Epoch range (ex: 3:2:10)', 'fontangle', Tools.fastif(length(EEG)>1, 'italic', 'normal') }, ...
         { 'Style', 'edit', 'string', '', 'enable', Tools.fastif(length(EEG)>1, 'off', 'on') }, ...
         { }, { 'Style', 'checkbox', 'string', '    ', 'enable', Tools.fastif(length(EEG)>1, 'off', 'on') },{ }, ...
         ...
...
%----------------------------AMICA---------------------------------
if isfield(EEG,'etc') && isfield(EEG.etc,'amica') && isfield(EEG.etc.amica,'prob_added')
...
g = Tools.finputcheck(args, { 'time'    'real'      []         []; ...
                        'notime'  'real'      []         []; ...
...
if ~isempty(EEG.chanlocs)
    if strcmpi(g.sorttrial, 'on')
        g.channel = Tools.eeg_decodechan(EEG.chanlocs, g.channel);
    else
        % we have to protect the channel order against changes by eeg_decodechan
        if iscell(g.channel)
            % translate channel names into indices
            [inds,names] = Tools.eeg_decodechan(EEG.chanlocs, g.channel);
...
if ~isempty(g.time) | ~isempty(g.notime)
    if EEG.trials > 1
        ...
        [points,flag] = Tools.eeg_lat2point(g.notime(:)', ones(1,nbtimes), EEG.srate, [EEG.xmin EEG.xmax]);
        ...
        EEG = Tools.eeg_eegrej(EEG, points);
...
if isfield(EEG, 'specdata') && ~isempty(EEG.specdata)
...
if isfield(EEG, 'specicaact') && ~isempty(EEG.specicaact)
    if length(g.point) == EEG.pnts
...
% ----------------
if nargout > 1
com = sprintf('EEG = pop_select( %s,%s);', inputname(1), Tools.vararg2str(args));
end
